...
// getDestinationNetworksAddressSetHashNames returns the hash names of the destination networks address set
// for a given EgressIP CR, or empty strings if no TrafficSelector is set.
func (e *EgressIPController) getDestinationNetworksAddressSetHashNames(egressIPName string, spec egressipv1.EgressIPSpec) (string, string, error) {
	if !hasTrafficSelector(spec) {
		return "", "", nil
	}
	dbIDs := getEgressIPDestNetworksAddrSetDbIDs(egressIPName, e.controllerName)
	...
	v4Name, v6Name := as.GetASHashNames()
	return v4Name, v6Name, nil
}
...
func (e *EgressIPController) addPodEgressIPAssignments(ni util.NetInfo, name string, statusAssignments []egressipv1.EgressIPStatusItem, mark util.EgressIPMark, pod *corev1.Pod) error {
	...
	if !exists {
		...
		for _, status := range statusAssignments {
			if _, exists := podState.egressStatuses.statusMap[status]; !exists {
				remainingAssignments = append(remainingAssignments, status)
			}
		}
		podState.standbyEgressIPNames.Delete(name)
	...
	for egressIPName, networkPods := range egressIPCache.egressIPNameToPods {
		for networkName, podCache := range networkPods {
			p1 := func(item *nbdb.LogicalRouterPolicy) bool {
				return (item.Priority == types.EgressIPReroutePriority || item.Priority == types.EgressIPRerouteDefaultPriority) &&
	...
	for eipName, networkCache := range cache.egressIPNameToPods {
		for networkName, data := range networkCache {
			...
			p := func(item *nbdb.LogicalRouterPolicy) bool {
				if (item.Priority != types.EgressIPReroutePriority && item.Priority != types.EgressIPRerouteDefaultPriority) || item.ExternalIDs[libovsdbops.NetworkKey.String()] != networkName {
					return false
				}
...
func (e *EgressIPController) addPodEgressIPAssignment(ni util.NetInfo, egressIPName string, status egressipv1.EgressIPStatusItem, mark util.EgressIPMark,
	...
	// Look up destination networks address set hash name for TrafficSelector-based LRP filtering.
	// If no address set exists (no TrafficSelector), destAddrSetHashName will be empty and the
	// reroute policy match will not include destination filtering.
	var destAddrSetHashName string
	isEgressIPv6 := utilnet.IsIPv6String(status.EgressIP)
	dbIDs := getEgressIPDestNetworksAddrSetDbIDs(egressIPName, e.controllerName)
	if destAS, err := e.addressSetFactory.GetAddressSet(dbIDs); err == nil && destAS != nil {
	...
	// Determine reroute priority. TrafficSelector EgressIPs use the standard priority.
	// Non-trafficSelector EgressIPs use a lower priority when coexisting with trafficSelector
	// EgressIPs, so the more specific destination-filtered rules are evaluated first.
	reroutePriority := types.EgressIPReroutePriority
	if destAddrSetHashName == "" {
		podKey := getPodKey(pod)
		if podState, exists := e.podAssignment.Load(podKey); exists && len(podState.egressStatuses.statusMap) > 0 {
			reroutePriority = types.EgressIPRerouteDefaultPriority
		}
	}
...
func (e *EgressIPController) deleteReroutePolicyOps(ni util.NetInfo, ops []ovsdb.Operation, status egressipv1.EgressIPStatusItem,
	...
	for _, priority := range []int{types.EgressIPReroutePriority, types.EgressIPRerouteDefaultPriority} {
...
func (e *EgressIPController) deleteEgressIPStatusSetup(ni util.NetInfo, name string, status egressipv1.EgressIPStatusItem) error {
	...
	policyPredNextHop := func(item *nbdb.LogicalRouterPolicy) bool {
		...
		return (item.Priority == types.EgressIPReroutePriority || item.Priority == types.EgressIPRerouteDefaultPriority) && hasIPNexthop &&
	...
	for _, transitIP := range transitRouterInfo.gatewayRouterNets {
		...
		ops, err := libovsdbops.ReplaceNextHopForLogicalRouterPolicyWithPredicateOps(
			e.nbClient, nil, func(policy *nbdb.LogicalRouterPolicy) bool {
				if policy.Priority != types.EgressIPReroutePriority && policy.Priority != types.EgressIPRerouteDefaultPriority {
					return false
				}
